...
class Equipment{
    ...
    public static function getListActionWorkingPowerFuel(){
        ...
        $result = $db->query("SELECT * FROM working_eqt");
        $result->setFetchMode(PDO::FETCH_ASSOC);
        $data['working'] = $result->fetchAll();

        $result = $db->query("SELECT * FROM power_eqt");
        $result->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public static function getListEquipment(){
        $db = Db::getConnection();
        $result = $db->query("SELECT * FROM equipment AS eq, action As ac, fuel_type AS fuel, working_eqt AS wor, power_eqt AS pow WHERE eq.action_id=ac.id_action AND eq.fuel_type_id=fuel.id_fuel AND eq.working_eqt_id=wor.id_working AND eq.power_eqt_id=pow.id_power AND NOT eq.status_delete = 1 ORDER BY id_equipment");
        $result->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public static function createWorking($working_name){
        $db = Db::getConnection();
        $db->query("INSERT INTO  working_eqt(name_working_ua) VALUE ('$working_name')");
    ...
    public static function createPower($power_name){
        $db = Db::getConnection();
        $db->query("INSERT INTO  power_eqt(name_power_ua) VALUE ('$power_name')");
    ...
    public static function createEquipment($working_eqt_id, $power_eqt_id, $action_id, $fuel_type_id, $fuel_cons, $productivity){
        $db = Db::getConnection();
        $db->query("INSERT INTO  equipment(action_id, working_eqt_id, power_eqt_id, fuel_type_id, 
    ...
    public static function getOneEquipment($id){
        $db = Db::getConnection();
        $result = $db->query("SELECT * FROM equipment WHERE id_equipment=$id");
        $result->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public static function editSaveEquipment($id_equipment, $working_eqt_id, $power_eqt_id, $action_id, $fuel_type_id, $fuel_cons, $productivity){
        ...
        $db->query("UPDATE equipment SET $save action_id='$action_id', working_eqt_id='$working_eqt_id', power_eqt_id='$power_eqt_id', fuel_type_id='$fuel_type_id' WHERE id_equipment = '$id_equipment'");
        return true;
    }
    ...
    public static function removeEquipment($id){
        $db = Db::getConnection();
        $db->query("UPDATE equipment SET status_delete = '1' WHERE id_equipment = $id");
        return true;
    }
    ...
    public static function getListWorking(){
        $db = Db::getConnection();
        $result = $db->query("SELECT * FROM working_eqt WHERE NOT id_working = 1 ");
        $result->setFetchMode(PDO::FETCH_ASSOC);
        $data = $result->fetchAll();
        return $data;
    }
    ...
    public static function getListPower(){
        $db = Db::getConnection();
        $result = $db->query("SELECT * FROM power_eqt WHERE NOT id_power = 1");
        $result->setFetchMode(PDO::FETCH_ASSOC);
        $data = $result->fetchAll();
        return $data;
    }
