...
class Users{
    ...
    public static function saveProfileFarmer ($id, $company_name, $specialization, $address, $position, $region, $zona){
        $db = Db::getConnection();

        $result = $db->query("UPDATE profile_farmer SET company_name = '$company_name', specialization = '$specialization', address = '$address', position = '$position', region = '$region' WHERE id_user = '$id'");
        if($result){
            $_SESSION['farmer_profile'] = true;
        }
        $db->query("UPDATE analytica_date SET zona=$zona WHERE id_user ='$id'");
        return true;
    }
    ...
    public static function getDateUser($id){
        ...
        $analytica_date = $db->query("SELECT * FROM analytica_date  WHERE id_user = '$id'");
        $analytica_date->setFetchMode(PDO::FETCH_ASSOC);
        $analytica = $analytica_date->fetchAll();
        if($analytica==false){
            $db->query("INSERT INTO analytica_date (id_user) VALUE ('$id')");
        }
