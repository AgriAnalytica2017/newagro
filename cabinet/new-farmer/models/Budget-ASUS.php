...
/*            array(
    ...
    public static function getNewBudget($db,$id_user,$field,$table,$remains=false,$strorage=false, $fact_remains=false,$crop_remains=false){
        $ex_date=array();
        $sql1='AND (';
        $sql2='AND (';
        $sql3='AND (';
        ...
        foreach ($field as $arr_field) {
            ...
            foreach ($date['new_action'] as $action)if($action['action_id_culture']==$arr_field['field_id_culture']){
                ...
                if(unserialize($action['action_materials'])!=false) foreach(unserialize($action['action_materials']) as $action_materials){
                    ...
                    if($date['new_material'][$action_materials['id']]==TRUE)$material_ok[$date['new_material'][$action_materials['id']]['id_lib_material']]=true;
                    if($remains==1){
                ...
                if(unserialize($action['action_machines'])!=false){
                    ...
                    foreach(unserialize($action['action_machines']) as $action_machines){
                        ...
                        if($date['new_material'][$date['vehicles'][$action_machines['id_veh']]['vehicles_fuel']]==TRUE)$material_ok[$date['new_material'][$date['vehicles'][$action_machines['id_veh']]['vehicles_fuel']]['id_lib_material']]=true;
        ...
        //Cash Flow
        $result = $db->query("SELECT * FROM new_cah_flow_material WHERE id_user='$id_user'");
        $result->setFetchMode(PDO::FETCH_ASSOC);
        ...
        foreach ($date['cash_flow_material'] as $arr_cash_flow_material){
            $arr_cash_flow_material['cah_flow_material']=unserialize($arr_cash_flow_material['cah_flow_material']);
            $month_d = explode('-', $arr_cash_flow_material['cah_flow_material']['d']);
            $month=intval($month_d[0].$month_d[1]);
            $ex_date['month_active'][$month]=true;
            $ex_date[$cf_type[0]][$month]=100;
            var_dump($ex_date['budget_seeds_month']);
            if($arr_cash_flow_material['cah_flow_material'][0]==TRUE){
