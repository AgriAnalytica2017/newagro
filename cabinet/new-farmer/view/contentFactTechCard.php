            ...
            if(fact['fact_machines'] == undefined || fact['fact_machines'] == false){
                ...
                $.each(fact['fact_machines'], function (key, machines) {
                    ...
                    if(storage[machines['id_mat']]!=undefined) {
                        $("#in_fact_equipment").append("<tr>"+
                            "<td>"+vehicles[machines['id_veh']]['vehicles_name']+"</td>"+
                            "<td id='equipment_in_"+id_machine+"'></td>"+
                            "<td>"+machines['norm']+"</td>"+
                            "<td>"+machines['date']+"</td>"+
                            "</tr>"
                        );
                            var id_equ = machines['id_eq'].split(',');
                            $.each(id_equ, function (key, id_equipment) {
                                $("#equipment_in_"+id_machine).append(equipment_lib[id_equipment]['equipment_name']+"<br>");
                            });
                        id_fuel++;
                        id_vehicles++;
                        /*$("#fact_action_fuel").append("<tr class='fuel_json' id='action_fuel_" + id_fuel + "'>" +
                        ...
                        jsonObj = [];
                        console.log(machines['id_eq']);
                        if (machines['id_eq']!=false){
                            console.log('on');
                            var vehicles_arr = machines['id_eq'].split(',');
                            var coll=0;
                            var text_eq='';
                            $.each(vehicles_arr, function(key2, value2) {
                                ...
                                item ['id'] = value2;
                                jsonObj.push(item);
                                text_eq+=equipment_lib[value2]['equipment_name']+', ';
                            });
                        }
                        $("#fact_action_fuel").append("<tr class='equipment_id fuel_json' id='action_vehicles_"+id_vehicles+"'>" +
                            "<td>"+vehicles[machines['id_veh']]['vehicles_name']+"</td>" +
                            "<td >" +
                            "<div class='btn-group'>" +
                            "<div id='vehicles_id_eq_"+id_vehicles+"'  data-id-vehicles='"+machines['id_veh']+"' data-id-equipment='"+JSON.stringify(jsonObj)+"'  class='ex_equ' style='max-width: 70px' disabled='disabled'>"+text_eq+"</div>" +
                            "<button data-id='"+id_vehicles+"' type='button' class='btn btn-primary open_equipment'>+</button>" +
                            "</div>" +
                            "</td>" +
                            "<td id='fuel_"+id_vehicles+"'>" +storage[machines['id_mat']]['name_material']+
                            "<br><button data-id='1' type='button' class='btn btn-primary open_fuel'>+</button>" +
                            "</td>" +
                            "<td id='norm_fuel_"+id_vehicles+"'>" +
                            "<input value='"+machines['norm']+"' data-id='"+machines['id_mat']+"' type='text' class='form-control norm_fuel fuel_st_"+machines['id_mat']+"' >" +
                            "</td>" +
                            "<td><input value='"+machines['date']+"' class='date_fuel form-control' type='date'></td>" +
                            "<td><button class='btn btn-danger btn-sm remove_vehicles' data-id='"+id_vehicles+"' ><i class='fa fa-fw fa-close'></i></button></td>" +
                            "</tr>");
